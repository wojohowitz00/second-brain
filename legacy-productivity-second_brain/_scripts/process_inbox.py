...
try:
    from keychain_helper import get_slack_credentials, get_llm_credentials
except ImportError:
    # Fallback if keychain_helper not available
...
def classify_thought(thought: str) -> dict:
    """
    Classify a thought using configured LLM provider.
    Returns classification dict with destination, confidence, filename, extracted fields.
    """
    provider = _get_llm_provider()
    
    if not provider:
        # Fallback to placeholder if no provider configured
        print("Warning: No LLM provider configured, using fallback classification")
        return {
            "destination": "ideas",
            "confidence": 0.5,
    ...
    try:
        classification = provider.classify(thought)
