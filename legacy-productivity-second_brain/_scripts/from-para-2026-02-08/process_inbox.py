...
VAULT_PATH = Path.home() / "PARA" / "1_projects" / "coding" / "second_brain"
...
- **USER HINT**: If text starts with "person:", "project:", "idea:", or "admin:", use that as the destination with HIGH confidence (0.9+) and strip the prefix from the content
...
def classify_thought(thought: str) -> dict:
    """
    Classify a thought using Claude API.
    Returns structured classification with destination, confidence, and metadata.
    """
    api_key = os.environ.get("ANTHROPIC_API_KEY")
    if not api_key or api_key == "your-anthropic-api-key-here":
        # Fallback if no API key configured
        print("⚠️  Warning: ANTHROPIC_API_KEY not configured, using fallback classification")
        return {
            "destination": "ideas",
            "confidence": 0.5,
            "filename": "unconfigured-fallback",
            "extracted": {"title": thought[:50]},
            "linked_entities": []
        }
    ...
    try:
        ...
        response = client.messages.create(
            ...
            messages=[{
                "role": "user",
                "content": CLASSIFICATION_PROMPT.format(thought=thought)
            }]
        )
...
def process_message(msg: dict) -> bool:
    ...
    # Handle trigger commands for summaries and health checks
    trigger = text.lower().strip()
    if trigger in ["digest", "review", "health"]:
        # Idempotency check for triggers
        if is_message_processed(ts):
            return True

        try:
            if trigger == "digest":
                from daily_digest import gather_active_items, generate_digest, find_stalled_items
                projects, people = gather_active_items()
                stalled = find_stalled_items(projects)
                response = generate_digest(projects, people, stalled)
            elif trigger == "review":
                from weekly_review import gather_week_data, generate_review
                inbox_logs, projects, stats = gather_week_data()
                response = generate_review(inbox_logs, projects, stats)
            elif trigger == "health":
                from health_check import check_health
                is_healthy, issues = check_health(max_age_minutes=60, alert=False)
                if is_healthy:
                    response = "✅ *System Health: HEALTHY*\n\nAll checks passed."
                else:
                    response = "⚠️ *System Health: UNHEALTHY*\n\nIssues:\n" + "\n".join(f"• {i}" for i in issues)

            reply_to_message(ts, response)
            ...
            reply_to_message(ts, f"❌ Error generating {trigger}: {str(e)}")
