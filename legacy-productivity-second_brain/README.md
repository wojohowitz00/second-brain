...
- **[📖 Usage Guide](USAGE.md)** - Complete guide to using the macOS app
- **[🚀 Quick Start](RUN_NOW.md)** - Get running in 5 minutes
- **[⚙️ Setup Guide](MANUAL_SETUP.md)** - Detailed setup instructions
...
### macOS
- macOS 12.0 or later (for GUI app) / macOS 10.15+ (for command line)
- Python 3.9+
...
- Xcode 14.0+ (for building GUI app)
...
**For private channels (like #wry_sb):**
...
**Note**: If your `#wry_sb` channel is private, you need both the `groups:*` scopes. If it's public, you need the `channels:*` scopes.
...
### Option 1: macOS GUI App (Recommended)

A native macOS app is available that provides a GUI for configuration, monitoring, and management.
...
2. **Configure in the app:**
   ...
   - Settings → LLM: Select provider and configure
...
See `SecondBrain/README.md` for more details.
...
The macOS app includes a built-in background service that replaces cron jobs:
- Automatic inbox processing every 2 minutes
- Fix handler runs every 5 minutes  
- Health checks every hour

Simply start the service from the Dashboard.

### Cron Jobs (Alternative)

**Note**: With trigger commands and the macOS app, cron jobs are now optional! You can manually post "digest", "review", or "health" to #wry_sb whenever you want, or use Slack Workflows to schedule them.

If you prefer traditional cron automation, add these to your crontab (`crontab -e`):

```cron
# Process inbox every 2 minutes
*/2 * * * * cd ~/SecondBrain/_scripts && source .env && uv run python3 process_inbox.py >> /tmp/wry_sb.log 2>&1

# Health check every hour
0 * * * * cd ~/SecondBrain/_scripts && source .env && uv run python3 health_check.py --quiet >> /tmp/wry_sb-health.log 2>&1

# Process fix commands every 5 minutes
*/5 * * * * cd ~/SecondBrain/_scripts && source .env && uv run python3 fix_handler.py >> /tmp/wry_sb-fix.log 2>&1
```
...
Post these keywords to `#wry_sb` for instant summaries (no cron jobs needed):
...
health
```
Bot replies with system status and any issues.

**Automation Tip**: Use Slack's Workflows to schedule these commands:
- Set up a daily workflow to post "digest" at 8am
- Set up a weekly workflow to post "review" on Monday mornings
...
cd ~/SecondBrain/_scripts
source .env
uv run python3 health_check.py
```

**Note**: This project uses `uv` for package management. If you haven't installed it yet:
```bash
