...
When a Slack message has file attachments, the app downloads them into the same vault folder as the note (1:1 mirrored). Allowed types: pdf, images (png, jpg, gif, webp), audio (mp3, m4a, wav), video (mp4, mov), text (txt, md, csv). Max size 50MB per file. The note body gets an "## Attachments" section with markdown links to the downloaded files. Implemented in [slack_client](apps/backend/_scripts/slack_client.py) (`get_message_files`, `download_file`), [file_writer](apps/backend/_scripts/file_writer.py) (`safe_attachment_filename`, `append_attachments_section`), and [process_inbox](apps/backend/_scripts/process_inbox.py) (`_process_attachments`).
...
├── scripts/SecondBrain.spec        # PyInstaller spec
...
Uses `scripts/SecondBrain.spec`:
- Entry point: `apps/backend/_scripts/menu_bar_app.py`
