...
cd apps/backend/_scripts && cat .env | head -3
...
1. **Verify token format:**
   ```bash
   cat apps/backend/_scripts/.env | grep SLACK_BOT_TOKEN
...
1. **Verify Channel ID:**
   ```bash
   cat apps/backend/_scripts/.env | grep SLACK_CHANNEL_ID
   # Should start with C
   ```
...
1. **Check message timestamp:**
   ...
   - State stored in `apps/backend/_scripts/.state/.last_processed_ts`

2. **Reset processing state:**
   ```bash
   rm apps/backend/_scripts/.state/.last_processed_ts
   ```
   Then click **Sync Now**.
...
cat apps/backend/_scripts/.state/.last_processed_ts
...
```bash
# Clear cache
rm apps/backend/_scripts/.state/vault_cache.json
```
...
2. **Process in batches:** If hundreds of messages backed up:
   ...
   date +%s > apps/backend/_scripts/.state/.last_processed_ts
   ```
...
| State | `apps/backend/_scripts/.state/` |
