...
def _get_token() -> str:
    """Get Slack token from environment or Keychain."""
    # Try Keychain first
    try:
        from keychain_helper import get_slack_credentials
...
def _get_channel_id() -> str:
    """Get channel ID from environment or Keychain."""
    # Try Keychain first
    try:
        from keychain_helper import get_slack_credentials
        creds = get_slack_credentials()
        channel_id = creds.get("SLACK_CHANNEL_ID")
        if channel_id:
            return channel_id
    except ImportError:
        pass
