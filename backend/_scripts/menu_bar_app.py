...
def open_note(path: str):
    ...
    """
    # Convert path to relative path within vault
    vault_path = Path.home() / "PARA"
    try:
        note_path = Path(path)
    ...
    # Build Obsidian URL
    # Format: obsidian://open?vault=PARA&file=path/to/file.md
    encoded_path = quote(str(relative_path).replace('.md', ''))
    obsidian_url = f"obsidian://open?vault=PARA&file={encoded_path}"
